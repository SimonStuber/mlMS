  ...
  modelBaseline <- nimbleCode({
    ...
    if(predX){
      ...
      if(fullAD){
        ...
        for(i in 1:N){
          xOutHat[i] <-  bb[1] + bb[2]*(b0[i]-mean(b0[1:N])) + bb[3]*(b1[i]-mean(b1[1:N])) + bb[4]*(eff[i,3]-mean(eff[1:N,3]))
          xOutcome[i] ~ dnorm(xOutHat[i], xOutResVar)
        }
      }
...
#
 ...
 #     initEffMeans <- c(fixef(LmerMod)[1],fixef(LmerMod)[2])
  ...
  monitorPars <- c("effMeans", "b1", "b0", "res", "eff", "effVar", "U")
