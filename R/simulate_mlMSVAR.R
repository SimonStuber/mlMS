...
simulate_mlMSVAR <- function(probs1,  n, nTime,nItems,
 ...
 # scaleResCov <- scaleResCov
  ...
  for(m in 1:M){
    ...
    i <- 1
    while(i <= n) {
      bRaneff[i,,m] <- mvtnorm::rmvnorm(1,b.means[,m], b.sd[,,m])
      b[i,,,m] <- bRaneff[i,,m]
  ...
  for(m in 1:n){
    ...
    for(t in 2:nTime){
     ...
     # browser()
      ...
      # if(s[t-1]==s[t]){
      y[t,m, ] <- t(b1[m,,s[t]] + b[m,,,s[t]]%*%(y[t-1,m,]-b1[m,,s[t]])) + mvtnorm::rmvnorm(1, rep(0,nItems), resCov[m,,,s[t]])
      #}else{
  ...
  res <- list(y=y,b1=b1,b=b,res=resCov, tp=tp, v=v, bRaneff=bRaneff, raneffs=v, b.means=b.means, b.sd=b.sd)
  return(res)
}
