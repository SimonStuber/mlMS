dForProbsHMM <- nimbleFunction(
  run = function(x=double(2),
    ...
    for(mm in 1:M){
      ...
      dens[1,mm] <- dmnorm_chol(x=y[1, 1:nVars], mean = y.hat[1, 1:nVars,mm], chol(resCov[1:nVars, 1:nVars,mm]), prec_param=FALSE, log=FALSE)
    }

    for(t in 2:nTime){
      for(mm in 1:M){

        y.hat[t, 1:nVars,mm] <- t(b1[1:nVars,mm])
...
registerDistributions(list("dForProbsHMM"=list(BUGSdist="dForProbsHMM(probs1,y.hat.err,b1,probs)",
