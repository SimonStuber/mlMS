...
fit_mlVAR <- function(simdat, randomEffects="transition_probabilities",
...
nVars <- nVars
    modelBaseline <- nimbleCode({
       for(i in 1:N){
           ...
           y[1:nTime, i, 1:nVars] ~ dVar(y.hat.err=resCov[i,1:nVars,1:nVars],
                                         b=bRaneff[i,1:(nVars*nVars)],
                                         b1=b1[i,1:nVars])
      ...
      for(i in 1:N){
         ...
         # allB[i,1:((nVars*nVars)+nVars),state] ~ dmnorm(b)
            bRaneff[i,1:(nVars*nVars)] ~ dmnorm(b.means[1:(nVars*nVars)], cov=b.prec[1:(nVars*nVars),1:(nVars*nVars)])
           # constraint_data ~ dconstraint(max(abs(bRaneff[i,1:(nVars*nVars),state]))< 1 )
          for(v in 1:nVars){
              b1[i,v] ~ dnorm(b1.means[v], var=b1.prec[v])
            }
      ...
      #     }
          ...
          for(item in 1:nVars){
            b1.means[item] ~ dnorm(0,0.0000001)
        ...
        for(eff in 1:(nVars*nVars)){
          b.means[eff] ~ T(dnorm(0,.001),-.99,.99)
        }
     ...
     # for(state in 1:M){
        b.prec[1:(nVars*nVars),1:(nVars*nVars)] ~ dinvwish(mat1[1:(nVars*nVars), 1:(nVars*nVars)],(nVars*nVars))
        for(v in 1:nVars){
          b1.prec[v] ~ dinvgamma(.1, .1)
        }
  ...
  #    for(i in 1:N){
       # for(state in 1:M){
        #  y.hat.err[i, state, 1:nVars] ~ dmnorm(resMean[i,state, 1:nVars], resCov[i, state, 1:nVars, 1:nVars])
          ...
          scaleResCov[1:nVars, 1:nVars] ~ dinvwish(matScale[1:nVars, 1:nVars], nVars)
          dfResCov ~ dunif(nVars+4, N*3)

         meanResCov[1:nVars, 1:nVars] <- scaleResCov[1:nVars, 1:nVars]/(dfResCov-nVars-1)
         for(i in 1:nVars){
           for(j in 1:nVars){
             varResCov[i,j] <- ((dfResCov-nVars+1)*(scaleResCov[i, j]^2)+(dfResCov-nVars-1)*scaleResCov[i, i]*scaleResCov[j,j])/
               ((dfResCov-nVars)*((dfResCov-nVars-1)^2)*(dfResCov-nVars-3))
           }
         }
         for(i in 1:N){
           resCov[i,1:nVars, 1:nVars] ~ dinvwish(scaleResCov[1:nVars, 1:nVars], dfResCov)
         }
       # }
   #   }
      ...
      # for(i in 1:N){
      #   #for(state in 1:M){
      #     resCov2[i,1:nVars,1:nVars] <-  resCov[i,1:nVars, 1:nVars]
      #   #}
      # }
...
nTime <- dim(simdat)[1]
   ...
   # matScale <- diag(nVars)
    ...
    if(is.null(inits)){
      inits <- init_mlVAR(simdat)
    }

#mat1 <- inits$b.prec
#matScale <- inits$scaleResCov
#for(state in 1:M){
  mat1 <- diag(diag(inits$b.prec))
  matScale <- diag(diag(inits$scaleResCov))
#}

dataList <- list("y"=simdat,
                 "mat1"=mat1,
                 "matScale"=matScale)
constantsList <- list("nTime"=dim(simdat)[1],
                      "N"=dim(simdat)[2],
                      "nVars"=nVars)
parameters <- c("dfResCov","b.means", "b1.means", "meanResCov", "varResCov", "bRaneff", "b1", "resCov")
...
clusterExport(cl, c("modelBaseline", "inits", "dataList", "constantsList","parameters",
                    ...
                    "neff.required",
                    "dVar", "makeMat", "makeSingleMat", "lse",
                    "init_mlVAR"),envir=environment())
...
AFSS_mcmcConfig$addSampler(type = 'slice',
                           target=c("dfResCov"),
    ...
    b1Target <- matrix(NA, constantsList$N)
    b1MeansTarget <- c()
    b1PrecTarget <- c()

    #for(state in 1:constantsList$M){
      for(i in 1:constantsList$N){
        b1Target[i] <- paste("b1[", i, ", 1:", constantsList$nVars, "]", sep="")
      }
    #}
   ...
   #   for(state in 1:constantsList$M){
    ...
    bRaneffTarget <- matrix(NA, constantsList$N)
    bMeansTarget <- c()
    bPrecTarget <- c()

   # for(state in 1:constantsList$M){
      for(i in 1:constantsList$N){
        bRaneffTarget[i] <- paste("bRaneff[",i, ", 1:", constantsList$nVars*constantsList$nVars,"]", sep="")
      }
    #}
    ...
    target <- array(NA, c(constantsList$N))
    for(i in 1:constantsList$N){
      #for(state in 1:constantsList$M){
        target[i] <- paste("resCov[", i, ", 1:", constantsList$nVars, ", 1:", constantsList$nVars, "]", sep="")
      #}
    }
    for(i in 1:constantsList$N){
      #for(state in 1:constantsList$M){
    ...
    scaleTarget <- c()
    #for(state in 1:constantsList$M){
      scaleTarget <- paste("scaleResCov[","1:",constantsList$nVars,", 1:",constantsList$nVars,"]", sep="")

      AFSS_mcmcConfig$addSampler(type = 'RW_wishart',
                                 target=scaleTarget,
                                 control=list("adaptInterval"=500,
                                              "scale"=mean(abs(inits$scaleResCov)),
                                              "adaptFactorExponent"=.5))
    #}

    AFSS_mcmcConfig$addMonitors(c('meanResCov', 'varResCov',"b1", "resCov", "bRaneff"))
