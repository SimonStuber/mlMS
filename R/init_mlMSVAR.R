...
#browser()
km.all <- kmeans(mlVarDat[,1:nVars],centers = M,nstart = nTime*n,iter.max = 500)

for(i in 1:n){

  km.res[[i]] <- try(kmeans(mlVarDat[mlVarDat$"ID"==i,1:nVars],centers = km.all$centers,nstart = nTime,
                   iter.max = 5000),silent = TRUE)
  ...
  for(state in 1:M){
    ...
    if(inherits(mlVarMod, "try-error")){
      if(nrow(newDat)<(nVars+1)){
        resCovInit[i,,,state] <- cov(mlVarDat[mlVarDat$"ID"==i,1:nVars])
        bRInits[i,,state] <- 0
      ...
      bRInits[i,,state] <- as.vector(arMat)
    ...
    resCovInit[i,,,state] <- solve(mlVarMod$kappa)

    bRInits[i,,state] <- as.vector(mlVarMod$beta[,(1:nVars)+1])
  ...
  for(state in 1:M){
    init_b.prec[,,state] <- cov(bRInits[,,state])
    init_b.prec[,,state] <- as.matrix(nearPD(init_b.prec[,,state])$mat)
  }
