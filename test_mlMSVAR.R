...
scaleResCov <- array(c(rwish_chol(1, diag(nVars)*2, nVars),
                      rwish_chol(1, diag(nVars)*2, nVars)),
                c(nVars,nVars,M))
...
apply(data$res, 2:4, var)
apply(data$res, 2:4, mean)
...
res <- fit_mlMSVAR(data$y, M = 2, niter = 12000, nVars=nVars,
